...
async def _fetch_all_from_spotify_in_chunks(fetch_function: Callable) -> List[dict]:
    ...
    output.extend([item.get('track') or item.get('item') for item in results['items'] if item.get('track') or item.get('item') is not None])
    ...
    if results['next']:
        ...
        for extra_result in extra_results:
            output.extend([item.get('track') or item.get('item') for item in extra_result['items'] if item.get('track') or item.get('item') is not None])
